...
const getResponseCorrectness = (correctResponse, lines, model, partialScores) => {
  ...
  lines.forEach((line) => {
    const isCorrectAnswer = correctExpressions.find((correctExpression) =>
      correctExpression.equals(line.expression || utils.expression(line.from, line.to)),
    );
...
export const getCorrectness = (question, session, env = {}) => {
  ...
  if (env.mode === 'evaluate') {
    ...
    return getResponseCorrectness(correctResponse, session.lines, question, partialScoring);
  }
};
