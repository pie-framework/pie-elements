...
describe('ordering', () => {
  ...
  describe('reducer', () => {
    ...
    describe.each([
      [simpleInput, [[0, 1]], moveAction, withoutTargets, [2, 1, 3, 4]],
      [
        simpleInput,
        [
          [0, 1],
      ...
      [shortInput, [[0, 3]], moveAction, allProps, [choice(1), choice(2), target(undefined, 0), target(1, 1)]],
      [shortInput, [[0, 3]], moveAction, allProps, { response: [undefined, 1] }],
      [shortInput, [[0, 2]], moveAction, allProps, { response: [1, undefined] }],
      [responseInput, [[3, 0]], moveAction, allProps, { response: [undefined, undefined] }],
      [responseInput, [[3, 1]], moveAction, allProps, { response: [undefined, undefined] }],
      [responseInput, [[3, 2]], moveAction, allProps, { response: [1, undefined] }],
      ...
      if (!opts.includeTargets) {
        ...
        it(label, () => {
          expect(state.tiles.map((t) => t.id).filter((item, index, array) => array.indexOf(item) === index)).toEqual(
            expected,
          );
        });
