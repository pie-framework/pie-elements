...
export class ModelOptions extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        <CardBar
            header="Configure Fraction Model"
          ></CardBar>
        <br/>
          ...
          <div className={classes.group}>
            ...
            <Select className={classes.container2} onChange={this.handleSelect} value={model.modelTypeSelected}>
              { modelTypeChoices.map((choice, index) => 
                  <MenuItem key={'item_'+index} value={choice.value}>{choice.label}</MenuItem>
                )}
            </Select>
          </div>
          <div className={classes.group}>
            <label className={classes.inputLabel}>Max # of Models</label>
            <MiniField min={maxOfModel.min} max={maxOfModel.max} 
              value={model.maxModelSelected} name="max-model" onChange={this.changeMaxModel} />
          </div>
          <div className={classes.group}>
        ...
        <div className={classes.checkbox}>
          <Checkbox 
            ...
            checked={model.allowedStudentConfig}
            label={''}
