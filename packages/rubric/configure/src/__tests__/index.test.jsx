...
describe('RubricElement', () => {
    ...
    describe('updateModelAccordingToReceivedProps', () => {
        ...
        it('should handle adding and removing elements from points and sampleAnswers arrays', () => {
            const originalModel = {
                maxPoints: 5,
                excludeZero: false,
                points: ['A', 'B', 'C'],
                sampleAnswers: [true, false, true]
            };
            const nextModel = {
                maxPoints: 3,
                excludeZero: true,
                points: ['A', 'B', 'C', 'D', 'E'],
                sampleAnswers: [true, false, true, false, true]
            };
            const result = element.updateModelAccordingToReceivedProps(nextModel);
            expect(result.points).toEqual(['B', 'C', 'D', 'E']);
            expect(result.sampleAnswers).toEqual([false, true, false, true]);
        });
    });
