...
function prepareForStatic(model, state) {
    const {expression, responses, printMode, alwaysShowCorrect, disabled} = model || {};
...
export class Main extends React.Component {
    ...
    constructor(props) {
        super(props);
        const { model } = props || {};
        // tODO there's no expression here
        const { expression, alwaysShowCorrect } = model || {};
    ...
    handleAnswerBlockDomUpdate = () => {
        ...
        const { disabled,view } = model || {};
        const {session, showCorrect} = this.state;
        const answers = session.answers;

        if (this.root && disabled && !showCorrect) {
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        // tODO we do not have expression
        let {note, language, env, expression, responses} = this.props.model;
        let {language: nextLanguage,
            env: nextEnv,
            alwaysShowCorrect: nextAlwaysShowCorrect,
        ...
        if (
            ...
            note = translator.t('mathInline.primaryCorrectWithAlternates', {lng: nextLanguage});
        }

        if ((env && env.mode !== 'evaluate') || (nextEnv && nextEnv !== 'evaluate')) {
        ...
        if (this.countResponseOccurrences(expression) < this.countResponseOccurrences(nextExpression)) {
            setTimeout(() => this.updateAria(), 100);
        }

        if (
            ( responses && Object.keys(responses || {}).length !== Object.keys(nextResponses || {}).length) ||
            // TODO ce-i cu conditia asta?
