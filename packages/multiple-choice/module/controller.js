...
const enabled = (config, env, defaultValue) => {
  ...
  return lodash.isBoolean(defaultValue) ? defaultValue : true;
};
...
const compact = arr => {
  if (Array.isArray(arr)) {
    return arr.filter(v => !isNull_1(v) && !isUndefined_1(v));
  }
  return arr;
};

const getShuffledChoices = (choices, session, updateSession, key) =>
  new Promise(resolve => {
  ...
  return new Promise(resolve => {
    if (!session || isEmpty_1(session)) {
      ...
      const partialScoringEnabled =
        enabled(model, env) && model.choiceMode !== 'radio';
